...
const Server = () => {
  ...
  useEffect(() => {
    fetch("https://server-site-azure.vercel.app/addprotfolio")
      .then((res) => res.json())
      .then((data) => {
        const filteredData = data.filter((item) => item.skill === "Laravel");
  ...
  return (
    <>
      {port.length > 0 ? (
        <div className="py-20">
          <p className="text-center lg:text-5xl text-3xl font-bold mb-6">
            Frontend Portfolio
          </p>
        ...
        <div className="py-20 text-center h-screen">
          <p className="text-2xl text-gray-600">No projects yet.</p>
        </div>
      )}
    </>
  );
